...
Unlike Western philosophy's focus on logic and analysis, Eastern philosophy emphasizes direct experience and practice, the interconnectedness of all things, balance and harmony, the nature of consciousness, and liberation from suffering.

Key traditions include Hinduism, Buddhism, Taoism, and Confucianism. These philosophies are meant to be lived and practiced daily.`,
          quiz: {
...
In Hinduism, dharma means cosmic law and order, one's duty and righteous path, the principles that sustain society, and living in alignment with one's nature and purpose.

Understanding your dharma means understanding your role in the cosmic order and how to live authentically.`,
          quiz: {
...
const SacredKnowledgeApp = () => {
  ...
  useEffect(() => {
    console.log('App loaded - ready to track progress');
  }, []);
  ...
  const completeChapter = (bookId, chapterId) => {
    ...
    if (!user.progress.chaptersCompleted.includes(key)) {
      setUser({
        ...user,
        progress: {
          ...user.progress,
          chaptersCompleted: [...user.progress.chaptersCompleted, key],
  ...
  const completeLesson = (courseId, lessonId) => {
    ...
    if (!user.progress.lessonsCompleted.includes(key)) {
      setUser({
        ...user,
        progress: {
          ...user.progress,
          lessonsCompleted: [...user.progress.lessonsCompleted, key],
